 ...
 *                 Project Name : WW3D                                                         *
 *                                                                                             *
 *                     $Archive:: /VSS_Sync/ww3d2/PartLdr.cpp              $*
 *                                                                                             *
...
ParticleEmitterDefClass::~ParticleEmitterDefClass ()
{
	...
	if (m_pName != nullptr) {
		...
		// free() is used because the buffer was allocated by the string helper.
		::free (m_pName);
		m_pName = nullptr;
	}
...
ParticleEmitterDefClass::Set_User_String (const char *pstring)
{
	...
	m_pUserString = duplicate_c_string (pstring);
}
...
{
	SAFE_FREE (m_pName);
	m_pName = duplicate_c_string (pname);
}
...
{
	copy_c_string (m_Info.TextureFilename, pname);
	Normalize_Filename ();
}
...
{
	char path[sizeof (m_Info.TextureFilename)];
	copy_c_string (path, m_Info.TextureFilename);

	// Find the last occurrence of the directory deliminator
	const char *filename = std::strrchr (path, '\\');
	const char *forward_slash = std::strrchr (path, '/');
	if (forward_slash != nullptr && (filename == nullptr || forward_slash > filename))
	...
	if (filename != nullptr) {
		...
		copy_c_string (m_Info.TextureFilename, filename + 1);
	}
}
...
ParticleEmitterDefClass::Read_Header (ChunkLoadClass &chunk_load)
{
	...
	if (chunk_load.Open_Chunk () &&
		...
		if (chunk_load.Read (&header, sizeof (header)) == sizeof (header)) {
			...
			m_pName = duplicate_c_string (header.Name);
...
ParticleEmitterDefClass::Save_Header (ChunkSaveClass &chunk_save)
{
	...
	if (chunk_save.Begin_Chunk (W3D_CHUNK_EMITTER_HEADER) == true) {
		...
		copy_c_string (header.Name, m_pName);
...
ParticleEmitterDefClass::Save_User_Data (ChunkSaveClass &chunk_save)
{
	...
	if (chunk_save.Begin_Chunk (W3D_CHUNK_EMITTER_USER_DATA) == true) {
		...
		unsigned int string_len = m_pUserString ? static_cast<unsigned int> (std::strlen (m_pUserString) + 1) : 0;
