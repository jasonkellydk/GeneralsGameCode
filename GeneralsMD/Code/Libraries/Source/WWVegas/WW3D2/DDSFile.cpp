...
namespace
{
	constexpr unsigned Make_FourCC(char a, char b, char c, char d)
	{
		return static_cast<unsigned>(static_cast<unsigned char>(a)) |
			(static_cast<unsigned>(static_cast<unsigned char>(b)) << 8) |
			(static_cast<unsigned>(static_cast<unsigned char>(c)) << 16) |
			(static_cast<unsigned>(static_cast<unsigned char>(d)) << 24);
	}

	WW3DFormat DDS_FourCC_To_WW3D_Format(unsigned four_cc)
	{
		switch (four_cc)
		{
		case Make_FourCC('D', 'X', 'T', '1'): return WW3D_FORMAT_DXT1;
		case Make_FourCC('D', 'X', 'T', '2'): return WW3D_FORMAT_DXT2;
		case Make_FourCC('D', 'X', 'T', '3'): return WW3D_FORMAT_DXT3;
		case Make_FourCC('D', 'X', 'T', '4'): return WW3D_FORMAT_DXT4;
		case Make_FourCC('D', 'X', 'T', '5'): return WW3D_FORMAT_DXT5;
		default: return WW3D_FORMAT_UNKNOWN;
		}
	}

	constexpr unsigned DDS_Caps2_CubeMap = 0x00000200u;
	constexpr unsigned DDS_Caps2_Volume = 0x00200000u;
}
...
// ----------------------------------------------------------------------------
//
// Copy one mipmap level to a caller-provided surface buffer.
//
// ----------------------------------------------------------------------------
