...
void W3DTerrainBackground::doPartialUpdate(const IRegion2D &partialRange, WorldHeightMap *htMap, Bool doTextures )
{
	...
	if (m_vertexTerrainSize<requiredVertexSize || m_vertexTerrain==nullptr) {
		...
		m_vertexTerrain=NEW_REF(VertexBufferClass,(RenderBackendVertexFormat::PositionDiffuseTexture,m_vertexTerrainSize+4,VertexBufferClass::USAGE_DEFAULT));
	}
	...
	if (m_indexTerrainSize<requiredIndexSize || m_indexTerrain==nullptr) {
		...
		REF_PTR_RELEASE(m_indexTerrain);
		m_indexTerrain=NEW_REF(IndexBufferClass,(m_indexTerrainSize+4,IndexBufferClass::USAGE_DEFAULT));
	}
	Int minX = m_xOrigin;
	Int minY = m_yOrigin;
	...
	UnsignedShort *ib;
	// Lock the buffer.
	VertexBufferClass::WriteLockClass lockVtxBuffer(m_vertexTerrain);
	...
	if (m_curNumTerrainIndices == 0) {
		...
		IndexBufferClass::WriteLockClass lockIdxBuffer(m_indexTerrain);
...
void W3DTerrainBackground::doTesselatedUpdate(const IRegion2D &partialRange, WorldHeightMap *htMap, Bool doTextures )
{
	...
	if (m_vertexTerrainSize<requiredVertex || m_vertexTerrain==nullptr) {
		...
		REF_PTR_RELEASE(m_vertexTerrain);
		m_vertexTerrain=NEW_REF(VertexBufferClass,(RenderBackendVertexFormat::PositionDiffuseTexture2,m_vertexTerrainSize+4,VertexBufferClass::USAGE_DEFAULT));
	}
