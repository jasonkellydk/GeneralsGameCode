...
// GENERALIZE FOR WHEN NO TEXTURE (DO NOT SET UV IN THESE CASES? NEED TO GENERALIZE FOR DIFFERENT TEXTURING MODES ANYWAY).
		...
		vArray[vidx].y = top.Y;
		vArray[vidx].z = top.Z;
		vArray[vidx].diffuse = WW3D::Get_Render_Backend()->Pack_Color(intersection[1][TOP_EDGE].RGBA);
		...
		vArray[vidx].diffuse = WW3D::Get_Render_Backend()->Pack_Color(intersection[1][BOTTOM_EDGE].RGBA);
		...
		for (; ; ) {
			...
			if (residual_top_points == 1 && residual_bottom_points == 1) {
				...
				vArray[vidx].y = top.Y;
				vArray[vidx].z = top.Z;
				vArray[vidx].diffuse = WW3D::Get_Render_Backend()->Pack_Color(intersection[top_int_idx][TOP_EDGE].RGBA);
				vArray[vidx].u1 = u_values[0] + uv_offset.X;
				...
				vArray[vidx].y = bottom.Y;
				vArray[vidx].z = bottom.Z;
				vArray[vidx].diffuse = WW3D::Get_Render_Backend()->Pack_Color(intersection[bottom_int_idx][BOTTOM_EDGE].RGBA);
				vArray[vidx].u1 = u_values[1] + uv_offset.X;
