 ...
 *                 Project Name : WW3D                                                         *
 *                                                                                             *
 *                     $Archive:: /Commando/Code/ww3d2/AggDef.cpp          $*
 *                                                                                             *
...
namespace
{
	void Copy_String(char *destination, std::size_t destination_size, const char *source)
	{
		...
		const std::size_t length = std::min(std::strlen(source), destination_size - 1);
...
AggregateDefClass::~AggregateDefClass ()
{
	...
	// free() is used because the buffer was allocated with WW3DString::Duplicate().
		::free (m_pName);
		m_pName = nullptr;
	}
...
)
{
	...
	for (int index = 1;
		...
		if (Equal_No_Case (ptemp_obj->Get_Name (), mesh_path[index])) {
					sub_obj = ptemp_obj;
				} else {
...
AggregateDefClass::Load_Assets (const char *passet_name)
{
	...
	if (passet_name != nullptr) {
		...
		if (!error)
		{
			path /= std::string(passet_name) + ".w3d";
			if (std::filesystem::exists(path, error) && !error)
			{
				const std::string path_string = path.string();
				retval = WW3DAssetManager::Get_Instance()->Load_3D_Assets(path_string.c_str());
			}
		}
	}
...
AggregateDefClass::Initialize (RenderObjClass &base_model)
{
	...
	Copy_String (m_Info.BaseModelName, sizeof (m_Info.BaseModelName), orig_model_name);
		...
		if (node_count > 0) {
			...
			for (int node_index = 0; node_index < node_count; node_index ++) {
				...
				if (psubobject != nullptr &&
					...
					Copy_String (subobj_info.SubobjectName, sizeof (subobj_info.SubobjectName), prototype_name);
					Copy_String (subobj_info.BoneName, sizeof (subobj_info.BoneName), pbone_name);
...
)
{
	...
	for (int node_index = 0; (node_index < node_list.Count ()) && (retval == false); node_index ++) {
		...
		if (prender_obj != nullptr &&
		    Equal_No_Case (prender_obj->Get_Name (), passet_name)) {
			retval = true;
		}
	}
...
AggregateDefClass::Read_Header (ChunkLoadClass &chunk_load)
{
	...
	if (chunk_load.Read (&header, sizeof (header)) == sizeof (header)) {
		...
		m_pName = WW3DString::Duplicate (header.Name);
...
AggregateDefClass::Add_Subobject (const W3dAggregateSubobjectStruct &subobj_info)
{
	...
	Copy_String (pnew_entry->SubobjectName, sizeof (pnew_entry->SubobjectName), subobj_info.SubobjectName);
	Copy_String (pnew_entry->BoneName, sizeof (pnew_entry->BoneName), subobj_info.BoneName);
...
AggregateDefClass::Save_Header (ChunkSaveClass &chunk_save)
{
	...
	if (chunk_save.Begin_Chunk (W3D_CHUNK_AGGREGATE_HEADER) == true) {
		...
		Copy_String (header.Name, sizeof (header.Name), m_pName);
