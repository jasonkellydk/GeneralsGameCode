...
SoundRenderObjDefClass::Write_Header (ChunkSaveClass &csave)
{
	...
	if (csave.Begin_Chunk (W3D_CHUNK_SOUNDROBJ_HEADER) == true) {
		...
		header.Flags	= Flags;
	const char *name = static_cast<const char *> (Name);
	const std::size_t name_length = std::min (std::strlen (name), sizeof (header.Name) - 1);
	std::memcpy (header.Name, name, name_length);
