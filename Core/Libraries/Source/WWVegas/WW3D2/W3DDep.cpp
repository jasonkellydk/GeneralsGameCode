 ...
 *                 Project Name : G                                                            *
 *                                                                                             *
 *                     $Archive:: /Commando/Code/ww3d2/W3DDep.cpp                            $*
 *                                                                                             *
 *                      $Author:: Byon_g                                                      $*
 *                                                                                             *
		...
		if (cload.Cur_Chunk_ID() == W3D_CHUNK_TEXTURE)
		{
			while (cload.Open_Chunk())
			{
				...
				if (cload.Cur_Chunk_ID() == W3D_CHUNK_TEXTURE_NAME)
				{
					...
					const std::size_t texture_length = static_cast<std::size_t>(cload.Cur_Chunk_Length());
					std::string texture(texture_length, '\0');
					cload.Read(texture.data(), cload.Cur_Chunk_Length());
					if (!texture.empty() && texture.front())	// don't push empty filenames
