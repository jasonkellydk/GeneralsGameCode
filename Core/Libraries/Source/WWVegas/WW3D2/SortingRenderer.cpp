	...
	unsigned short vertex_count)
{
	if (!WW3D::Is_Sorting_Enabled()) {
		WW3D::Get_Render_Backend()->Draw_Indexed_Primitives(
			RenderBackendPrimitiveType::TriangleList, 0, min_vertex_index,
			vertex_count, start_index, polygon_count);
		return;
	}
	...
	WW3D::Get_Render_Backend()->Capture_Render_State(state->sorting_state);
	...
	if (bounding_sphere.Is_Valid())
	{
		...
		Matrix4x4::Transform_Vector(mtx,bounding_sphere.Center,&transformed_vec);
...
{
	WW3D::Get_Render_Backend()->Set_Shader(ShaderClass(render_state.shader_bits));
	...
	WW3D::Get_Render_Backend()->Set_Transform(RenderBackendTransform::World,render_state.world);
	WW3D::Get_Render_Backend()->Set_Transform(RenderBackendTransform::View,render_state.view);
  ...
  //prevLight = render_state.lightsHash;
	...
	if (render_state.LightEnable[0]) {
		WW3D::Get_Render_Backend()->Set_Light_From_State(0, &render_state.Lights[0]);
		if (render_state.LightEnable[1]) {
			WW3D::Get_Render_Backend()->Set_Light_From_State(1, &render_state.Lights[1]);
			if (render_state.LightEnable[2]) {
				WW3D::Get_Render_Backend()->Set_Light_From_State(2, &render_state.Lights[2]);
				if (render_state.LightEnable[3]) {
					WW3D::Get_Render_Backend()->Set_Light_From_State(3, &render_state.Lights[3]);
				}
				else {
...
void SortingRendererClass::Flush_Sorting_Pool()
{
	...
	DynamicVBAccessClass dyn_vb_access(BUFFER_TYPE_DYNAMIC_RENDER,RenderBackend_Dynamic_Vertex_Format,vertexAllocCount/*overlapping_vertex_count*/);
	{
	...
	while (chunkOffset < overlapping_polygon_count)
	{
		...
		DynamicIBAccessClass dyn_ib_access(BUFFER_TYPE_DYNAMIC_RENDER,chunkCount*3);
		{
...
void SortingRendererClass::Flush()
{
	...
	while (!sorted_list.empty()) {
		...
		if ((state->sorting_state.index_buffer_type==BUFFER_TYPE_SORTING || state->sorting_state.index_buffer_type==BUFFER_TYPE_DYNAMIC_SORTING) &&
			...
			WW3D::Get_Render_Backend()->Apply_Render_State(state->sorting_state);
			WW3D::Get_Render_Backend()->Draw_Indexed_Primitives(
				RenderBackendPrimitiveType::TriangleList, 0,
				state->min_vertex_index, state->vertex_count,
				state->start_index, state->polygon_count);
			WW3D::Get_Render_Backend()->Restore_Render_State();
			Release_Refs(state);
			clean_list.push_front(state);
		}
	}
