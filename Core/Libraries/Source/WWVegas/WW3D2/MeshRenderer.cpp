 ...
 *                 Project Name : ww3d                                                         *
 *                                                                                             *
 *                     $Archive:: /Commando/Code/ww3d2/meshrenderer.cpp                        $*
 *                                                                                             *
...
/*
** Global Instance of the MeshMeshRender
*/
MeshRendererClass TheMeshRenderer;
bool MeshTextureCategoryClass::m_gForceMultiply = false; // Forces opaque materials to use the multiply blend - pseudo transparent effect.  jba.
// ----------------------------------------------------------------------------
...
// Mesh batching predates IRenderBackend and used a fixed vertex-format bitmask
// as its category key. Keep the key private to this shared batching
// implementation; it is deliberately not a native graphics API value anymore.
namespace
{
constexpr unsigned VERTEX_KEY_NORMAL = 1u << 0;
constexpr unsigned VERTEX_KEY_DIFFUSE = 1u << 1;
constexpr unsigned VERTEX_KEY_SPECULAR = 1u << 2;
constexpr unsigned VERTEX_KEY_TEXTURE_SHIFT = 8;
constexpr unsigned VERTEX_KEY_TEXTURE_MASK = 0xFFu << VERTEX_KEY_TEXTURE_SHIFT;

inline bool Vertex_Key_Has_Normal(unsigned key)
{
	return (key & VERTEX_KEY_NORMAL) != 0;
}

inline bool Vertex_Key_Has_Diffuse(unsigned key)
{
	return (key & VERTEX_KEY_DIFFUSE) != 0;
}

inline bool Vertex_Key_Has_Specular(unsigned key)
{
	return (key & VERTEX_KEY_SPECULAR) != 0;
}

inline unsigned Vertex_Key_Texture_Count(unsigned key)
{
	return (key & VERTEX_KEY_TEXTURE_MASK) >> VERTEX_KEY_TEXTURE_SHIFT;
}

inline unsigned Make_Vertex_Key(bool normal, bool diffuse, bool specular, unsigned texture_count)
{
	return (normal ? VERTEX_KEY_NORMAL : 0u) |
		(diffuse ? VERTEX_KEY_DIFFUSE : 0u) |
		(specular ? VERTEX_KEY_SPECULAR : 0u) |
		((texture_count & 0xFFu) << VERTEX_KEY_TEXTURE_SHIFT);
}

RenderBackendVertexLayout Vertex_Layout_From_Key(unsigned key)
{
	const bool normal = Vertex_Key_Has_Normal(key);
	const bool diffuse = Vertex_Key_Has_Diffuse(key);
	const unsigned texture_count = Vertex_Key_Texture_Count(key);

	RenderBackendVertexFormat format = RenderBackendVertexFormat::Unknown;
	if (normal) {
		switch (texture_count) {
		case 0: format = diffuse ? RenderBackendVertexFormat::PositionNormalDiffuse : RenderBackendVertexFormat::PositionNormal; break;
		case 1: format = diffuse ? RenderBackendVertexFormat::PositionNormalDiffuseTexture : RenderBackendVertexFormat::PositionNormalTexture; break;
		case 2: format = diffuse ? RenderBackendVertexFormat::PositionNormalDiffuseTexture2 : RenderBackendVertexFormat::PositionNormalTexture2; break;
		default: break;
		}
	} else {
		switch (texture_count) {
		case 0: format = diffuse ? RenderBackendVertexFormat::PositionDiffuse : RenderBackendVertexFormat::Position; break;
		case 1: format = diffuse ? RenderBackendVertexFormat::PositionDiffuseTexture : RenderBackendVertexFormat::PositionTexture; break;
		case 2: format = diffuse ? RenderBackendVertexFormat::PositionDiffuseTexture2 : RenderBackendVertexFormat::PositionTexture2; break;
		default: break;
		}
	}

	RenderBackendVertexLayout layout = RenderBackend_Vertex_Layout(format);
	if (format == RenderBackendVertexFormat::Unknown) {
		layout.format = RenderBackendVertexFormat::Unknown;
		layout.has_normal = normal;
		layout.has_diffuse = diffuse;
		layout.texture_count = texture_count < RENDER_BACKEND_MAX_TEXTURE_COORDINATES ?
			texture_count : RENDER_BACKEND_MAX_TEXTURE_COORDINATES;
		for (unsigned index = 0; index < layout.texture_count; ++index) {
			layout.texture_dimensions[index] = 2;
		}
	}
	layout.has_specular = Vertex_Key_Has_Specular(key);
	return layout;
}
...
{
public:
	MeshTextureCategoryClass *	src;
	MeshTextureCategoryClass *	dest;
	MeshPolygonRendererClass *  pr;
};
...
** (and therefore their mesh polygon renderers) this record contains
...
{
public:
	PolyRenderTaskClass(MeshPolygonRendererClass * p_renderer,MeshClass * p_mesh) :
		Renderer(p_renderer),
		Mesh(p_mesh),
		NextVisible(nullptr)
	{
	...
	MeshPolygonRendererClass *	Peek_Polygon_Renderer()							{ return Renderer; }
...
protected:
	...
	MeshPolygonRendererClass *	Renderer;
	MeshClass *						Mesh;
...
MeshTextureCategoryClass::MeshTextureCategoryClass(
	MeshFVFCategoryContainer* container_,
	TextureClass** texs,
	ShaderClass shd,
...
{
	WWASSERT(pass>=0);
	WWASSERT(pass<MeshFVFCategoryContainer::MAX_PASSES);
...
MeshTextureCategoryClass::~MeshTextureCategoryClass()
{
	...
	while (MeshPolygonRendererClass* p_renderer=PolygonRendererList.Get_Head()) {
		TheMeshRenderer.Unregister_Mesh_Type(p_renderer->Get_Mesh_Model_Class());
	}
	for (int a=0;a<MeshMatDescClass::MAX_TEX_STAGES;++a)
	{
	...
	DEBUG_ASSERTCRASH(render_task_head == nullptr, ("~MeshTextureCategoryClass: Leaking render tasks"));
}

void MeshTextureCategoryClass::Add_Render_Task(MeshPolygonRendererClass * p_renderer,MeshClass * p_mesh)
{
...
void MeshTextureCategoryClass::Add_Polygon_Renderer(MeshPolygonRendererClass* p_renderer,MeshPolygonRendererClass* add_after_this)
{
...
void MeshTextureCategoryClass::Remove_Polygon_Renderer(MeshPolygonRendererClass* p_renderer)
{
...
void MeshFVFCategoryContainer::Remove_Texture_Category(MeshTextureCategoryClass* tex_category)
{
	unsigned pass=0;
...
void MeshFVFCategoryContainer::Add_Visible_Material_Pass(MaterialPassClass * pass,MeshClass * mesh)
{
...
void MeshFVFCategoryContainer::Render_Procedural_Material_Passes()
{
	// additional passes
...
void MeshRigidFVFCategoryContainer::Add_Delayed_Visible_Material_Pass(MaterialPassClass * pass, MeshClass * mesh)
{
	MatPassTaskClass * new_mpr = new MatPassTaskClass(pass,mesh);
...
void MeshRigidFVFCategoryContainer::Render_Delayed_Procedural_Material_Passes()
{
	...
	SNAPSHOT_SAY(("MeshRigidFVFCategoryContainer::Render_Delayed_Procedural_Material_Passes()"));
...
void MeshTextureCategoryClass::Log(bool only_visible)
{
#ifdef ENABLE_CATEGORY_LOG
	StringClass work(255,true);
	work.Format("	MeshTextureCategoryClass");
	...
	MeshPolygonRendererListIterator it(&PolygonRendererList);
	while (!it.Is_Done()) {

		MeshPolygonRendererClass* p_renderer = it.Peek_Obj();
...
MeshFVFCategoryContainer::MeshFVFCategoryContainer(unsigned FVF_,bool sorting_)
	:
	FVF(FVF_),
	sorting(sorting_),
	...
	AnyDelayedPassesToRender(false)
{
	uv_coordinate_channels = Vertex_Key_Texture_Count(FVF);
}

// ----------------------------------------------------------------------------

MeshFVFCategoryContainer::~MeshFVFCategoryContainer()
{
	REF_PTR_RELEASE(index_buffer);

	for (unsigned p=0;p<passes;++p) {
		while (MeshTextureCategoryClass * tex = texture_category_list[p].Remove_Head()) {
			delete tex;
		}
	}
}
...
MeshTextureCategoryClass* MeshFVFCategoryContainer::Find_Matching_Texture_Category(
	...
	MeshTextureCategoryClass* ref_category)
{
	...
	MeshTextureCategoryClass* dest_tex_category=nullptr;
...
void MeshFVFCategoryContainer::Change_Polygon_Renderer_Texture(
	...
	unsigned stage)
{
	...
	while (!src_it.Is_Done()) {
		MeshTextureCategoryClass* src_tex_category=src_it.Peek_Obj();
		if (src_tex_category->Peek_Texture(stage)==texture) {
			foundtexture=true;
			MeshPolygonRendererListIterator poly_it(&polygon_renderer_list);
			while (!poly_it.Is_Done()) {
				// If source texture category contains polygon renderer, move to destination category
				MeshPolygonRendererClass* polygon_renderer=poly_it.Peek_Obj();
				MeshTextureCategoryClass *prc=polygon_renderer->Get_Texture_Category();

				if (prc==src_tex_category) {
					MeshTextureCategoryClass* dest_tex_category=Find_Matching_Texture_Category(new_texture,pass,stage,src_tex_category);
					...
					if (!dest_tex_category) {
						...
						MeshTextureCategoryClass * new_tex_category=W3DNEW MeshTextureCategoryClass(
							this,
							tmp_textures,
...
void MeshFVFCategoryContainer::Change_Polygon_Renderer_Material(
		MeshPolygonRendererList& polygon_renderer_list,
		VertexMaterialClass* vmat,
	...
	while (!src_it.Is_Done()) {
		...
		if (src_tex_category->Peek_Material()==vmat) {
			...
			while (!poly_it.Is_Done()) {
				...
				if (prc==src_tex_category) {
					foundtexture=true;
					MeshTextureCategoryClass* dest_tex_category=Find_Matching_Texture_Category(new_vmat,pass,src_tex_category);
...
unsigned MeshFVFCategoryContainer::Define_FVF(MeshModelClass* mmc,bool enable_lighting)
{
	if ((!!mmc->Get_Flag(MeshGeometryClass::SORT)) && WW3D::Is_Sorting_Enabled()) {
		return Make_Vertex_Key(true, true, false, 2);
	}

	int tex_coord_count=mmc->Get_UV_Array_Count();
	const unsigned texture_count = tex_coord_count < 0 ? 0u :
		static_cast<unsigned>(tex_coord_count) > RENDER_BACKEND_MAX_TEXTURE_COORDINATES ?
			RENDER_BACKEND_MAX_TEXTURE_COORDINATES : static_cast<unsigned>(tex_coord_count);
	const bool diffuse = mmc->Get_Color_Array(0,false) != nullptr;
	const bool specular = mmc->Get_Color_Array(1,false) != nullptr;
	const bool normal = mmc->Needs_Vertex_Normals();
	(void)enable_lighting;
	return Make_Vertex_Key(normal, diffuse, specular, texture_count);
}

// ----------------------------------------------------------------------------

MeshRigidFVFCategoryContainer::MeshRigidFVFCategoryContainer(unsigned FVF,bool sorting_)
	:
	MeshFVFCategoryContainer(FVF,sorting_),
...
MeshRigidFVFCategoryContainer::~MeshRigidFVFCategoryContainer()
{
...
void MeshRigidFVFCategoryContainer::Log(bool only_visible)
{
#ifdef ENABLE_CATEGORY_LOG
	StringClass work(255,true);
	work.Format("MeshRigidFVFCategoryContainer --------------");
	WWDEBUG_SAY((work));
	if (vertex_buffer) {
...
void MeshRigidFVFCategoryContainer::Render()
{
	...
	SNAPSHOT_SAY(("MeshRigidFVFCategoryContainer::Render()"));
	for (unsigned p=0;p<passes;++p) {
		SNAPSHOT_SAY(("Pass: %d",p));
		while (MeshTextureCategoryClass * tex = visible_texture_category_list[p].Remove_Head()) {
			tex->Render();
		}
	}
...
bool MeshRigidFVFCategoryContainer::Check_If_Mesh_Fits(MeshModelClass* mmc)
{
...
void MeshRigidFVFCategoryContainer::Add_Mesh(MeshModelClass* mmc_)
{
	...
	*/
	...
	for (i=0; i<split_table.Get_Vertex_Count(); i++)
	{
		*(Vector3*)(vb+fi.Get_Location_Offset())=locs[i];
		...
		if (Vertex_Key_Has_Normal(FVF) && norms) {
			*(Vector3*)(vb+fi.Get_Normal_Offset())=norms[i];
		}

		if (Vertex_Key_Has_Diffuse(FVF)) {
			if (diffuse) {
		...
		if (Vertex_Key_Has_Specular(FVF)) {
			if (specular) {
	...
	*/
	int uvcount = static_cast<int>(Vertex_Key_Texture_Count(FVF));
...
void MeshFVFCategoryContainer::Insert_To_Texture_Category(
...
{
	/*
	** Try to find a MeshTextureCategoryClass in this FVF container which matches the
	...
	*/
	...
	while (!it.Is_Done()) {
		MeshTextureCategoryClass * tex_category=it.Peek_Obj();
		// Compare all stage's textures
		bool all_textures_same = true;
	...
	if (!fit_in_existing_category) {
		...
		MeshTextureCategoryClass * new_tex_category=W3DNEW MeshTextureCategoryClass(this,texs,shader,mat,pass);
...
void MeshFVFCategoryContainer::Generate_Texture_Categories(Vertex_Split_Table& split_table,unsigned vertex_offset)
{
...
MeshSkinFVFCategoryContainer::MeshSkinFVFCategoryContainer(bool sorting)
	:
	MeshFVFCategoryContainer(Make_Vertex_Key(true, false, false, 1),sorting),
...
MeshSkinFVFCategoryContainer::~MeshSkinFVFCategoryContainer()
{
}

// ----------------------------------------------------------------------------

void MeshSkinFVFCategoryContainer::Log(bool only_visible)
{
#ifdef ENABLE_CATEGORY_LOG
	StringClass work(255,true);
	work.Format("MeshSkinFVFCategoryContainer --------------");
	WWDEBUG_SAY((work));
...
void MeshSkinFVFCategoryContainer::Render()
{
	SNAPSHOT_SAY(("MeshSkinFVFCategoryContainer::Render()"));
	...
	for (unsigned pass=0;pass<passes;++pass) {
		while (MeshTextureCategoryClass * tex = visible_texture_category_list[pass].Remove_Head()) {
		}
	}
...
bool MeshSkinFVFCategoryContainer::Check_If_Mesh_Fits(MeshModelClass* mmc)
{
...
void MeshSkinFVFCategoryContainer::clearVisibleSkinList()
{
...
void MeshSkinFVFCategoryContainer::Add_Visible_Skin(MeshClass * mesh)
{
...
void MeshSkinFVFCategoryContainer::Reset()
{
	clearVisibleSkinList();

	for (unsigned pass=0;pass<passes;++pass) {
		while (MeshTextureCategoryClass* texture_category=texture_category_list[pass].Peek_Head()) {
...
void MeshSkinFVFCategoryContainer::Add_Mesh(MeshModelClass* mmc)
{
...
unsigned MeshTextureCategoryClass::Add_Mesh(
	Vertex_Split_Table& split_table,
	unsigned vertex_offset,
	...
	unsigned pass)
{
	...
	*/
	...
	/*
	** Add the indices for the polygons that match into this renderer's mesh index table.
	*/
	if (polygons) {
		...
		if (stripify) {
			...
			if (index_count<unsigned(strip[0])) {
				...
				MeshPolygonRendererClass* p_renderer=W3DNEW MeshPolygonRendererClass(
					index_count,
...
void MeshTextureCategoryClass::Render()
{
	#ifdef WWDEBUG
	...
	while (prt) {
		...
		*/
		MeshPolygonRendererClass * renderer = prt->Peek_Polygon_Renderer();
		...
		*/
		...
		if (mesh->Peek_Model()->Get_Flag(MeshModelClass::ALIGNED)) {
			...
			TheMeshRenderer.Peek_Camera()->Get_Transform().Get_Z_Vector(&camera_z_vector);
			...
			TheMeshRenderer.Peek_Camera()->Get_Transform().Get_Translation(&camera_position);
			mesh->Get_Transform().Get_Translation(&mesh_position);
...
//--------------------------------------------------------------------
		...
		*/
		...
		if (!RendererDebuggerClass::Is_Enabled() || !mesh->Is_Disabled_By_Debugger()) {
...
void MeshTextureCategoryClass::Clear_Render_List()
{
...
MeshRendererClass::MeshRendererClass()
	:
	camera(nullptr),
...
void MeshRendererClass::Shutdown()
{
	camera = nullptr;
	visible_decal_meshes = nullptr;
	Invalidate(true);
...
void MeshRendererClass::Clear_Pending_Delete_Lists()
{
	while (MeshTextureCategoryClass* category=texture_category_delete_list.Remove_Head()) {
		delete category;
	}
	while (MeshFVFCategoryContainer* container=fvf_category_container_delete_list.Remove_Head()) {
		delete container;
	}
}
...
static void Add_Rigid_Mesh_To_Container(FVFCategoryList* container_list,unsigned fvf,MeshModelClass* mmc)
{
	...
	MeshFVFCategoryContainer * container = nullptr;
	...
	container=W3DNEW MeshRigidFVFCategoryContainer(fvf,sorting);
...
void MeshRendererClass::Unregister_Mesh_Type(MeshModelClass* mmc)
{
	MeshModelRenderData * mesh_data = Get_Mesh_Model_Render_Data(mmc);
	if (mesh_data != nullptr) {
		while (MeshPolygonRendererClass* n=mesh_data->PolygonRendererList.Remove_Head()) {
			delete n;
		}
		delete mesh_data;
...
void MeshRendererClass::Register_Mesh_Type(MeshModelClass* mmc)
{
	if (mmc->Get_Render_Backend_Data() == nullptr) {
		mmc->Set_Render_Backend_Data(W3DNEW MeshModelRenderData);
	}
	...
	if (skin) {
		...
		*/
		...
		MeshFVFCategoryContainer * new_container=W3DNEW MeshSkinFVFCategoryContainer(sorting);
		...
		*/
		if (!_RegisteredMeshList.Contains(mmc)) {
			...
			unsigned fvf=MeshFVFCategoryContainer::Define_FVF(mmc,enable_lighting);
			...
			*/
			...
			for (;i<texture_category_container_lists_rigid.Count();++i) {
				...
				WWASSERT(list);
				MeshFVFCategoryContainer * container=list->Peek_Head();
...
bool MeshRendererClass::Has_Mesh_Renderers(const MeshModelClass * mmc) const
{
	const MeshModelRenderData * mesh_data = Get_Mesh_Model_Render_Data(mmc);
	return mesh_data != nullptr && !const_cast<MeshPolygonRendererList &>(
		mesh_data->PolygonRendererList).Is_Empty();
}

unsigned MeshRendererClass::Get_Mesh_Renderer_Vertex_Offset(const MeshModelClass * mmc) const
{
	const MeshModelRenderData * mesh_data = Get_Mesh_Model_Render_Data(mmc);
	if (mesh_data == nullptr || const_cast<MeshPolygonRendererList &>(
		mesh_data->PolygonRendererList).Peek_Head() == nullptr) {
		return 0;
	}
	return const_cast<MeshPolygonRendererList &>(mesh_data->PolygonRendererList).Peek_Head()->Get_Vertex_Offset();
}

unsigned MeshRendererClass::Get_Mesh_Renderer_Count(const MeshModelClass * mmc) const
{
	const MeshModelRenderData * mesh_data = Get_Mesh_Model_Render_Data(mmc);
	return mesh_data == nullptr ? 0 : const_cast<MeshPolygonRendererList &>(
		mesh_data->PolygonRendererList).Count();
}

void MeshRendererClass::Update_Mesh_Texture(MeshModelClass * mmc, TextureClass * texture,
	TextureClass * new_texture, unsigned pass, unsigned stage)
{
	...
	MeshTextureCategoryClass * category =
...
void MeshRendererClass::Update_Mesh_Material(MeshModelClass * mmc,
...
void MeshRendererClass::Add_Mesh_Render_Tasks(MeshModelClass * mmc, MeshClass * instance)
{
	MeshModelRenderData * mesh_data = Get_Mesh_Model_Render_Data(mmc);
	if (mesh_data == nullptr) {
		return;
	}
	MeshPolygonRendererListIterator iterator(&mesh_data->PolygonRendererList);
	while (!iterator.Is_Done()) {
		MeshPolygonRendererClass * polygon_renderer = iterator.Peek_Obj();
		polygon_renderer->Get_Texture_Category()->Add_Render_Task(polygon_renderer, instance);
		iterator.Next();
	}
}

void MeshRendererClass::Add_Mesh_Material_Pass(MeshModelClass * mmc,
...
void MeshRendererClass::Add_Mesh_Skin(MeshModelClass * mmc, MeshClass * instance)
{
	MeshModelRenderData * mesh_data = Get_Mesh_Model_Render_Data(mmc);
	if (mesh_data == nullptr || mesh_data->PolygonRendererList.Peek_Head() == nullptr) {
		return;
	...
	MeshTextureCategoryClass * category =
		mesh_data->PolygonRendererList.Peek_Head()->Get_Texture_Category();
	if (category != nullptr && category->Get_Container() != nullptr) {
		static_cast<MeshSkinFVFCategoryContainer *>(category->Get_Container())->Add_Visible_Skin(instance);
	}
}

void MeshRendererClass::Render_Mesh_Pass(MeshModelClass * mmc, int base_vertex_offset)
{
	MeshModelRenderData * mesh_data = Get_Mesh_Model_Render_Data(mmc);
	if (mesh_data == nullptr) {
		return;
...
void MeshRendererClass::Request_Log_Statistics()
{
...
void MeshRendererClass::Flush()
{
	...
	WWPROFILE("MeshMeshRenderer::Flush");
	if (!camera) return;
...
void MeshRendererClass::Add_To_Render_List(DecalMeshClass * decalmesh)
{
...
void MeshRendererClass::Render_Decal_Meshes()
{
...
void MeshRendererClass::Log_Statistics_String(bool only_visible)
{
...
{
	while (MeshFVFCategoryContainer* fvf_category=list.Remove_Head()) {
		delete fvf_category;
	}
}
