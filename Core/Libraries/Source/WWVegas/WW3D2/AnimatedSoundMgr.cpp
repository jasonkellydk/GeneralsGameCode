...
)
{
		 ...
		 (string_list != nullptr))
	{
			  ...
			  entry = ::strstr (entry, delimiter))
		{
			...
			//
			if ((WW3DString::Compare_No_Case_N (entry, delimiter, delim_len) == 0) && (count > 0)) {
				entry += delim_len;
			}
...
AnimatedSoundMgrClass::Initialize (const char *ini_filename)
{
	...
	WW3DString::To_Upper (animation_name.Peek_Buffer ());
...
AnimatedSoundMgrClass::Find_Sound_List (HAnimClass *anim)
{
	...
	//
	WW3DString::To_Upper (full_name.Peek_Buffer ());
