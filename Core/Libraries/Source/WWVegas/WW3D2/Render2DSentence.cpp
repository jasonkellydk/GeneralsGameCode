...
////////////////////////////////////////////////////////////////////////////////////
Vector2
Render2DSentenceClass::Get_Formatted_Text_Extents (const wchar_t *text)
{
...
Render2DSentenceClass::Build_Textures ()
{
	...
	for (int index = 0; index < PendingSurfaces.Count (); index ++) {
		...
		//
		WW3D::Get_Render_Backend()->Copy_Surface(curr_surface, texture_surface);
...
////////////////////////////////////////////////////////////////////////////////////
void
Render2DSentenceClass::Allocate_New_Surface (const wchar_t *text, bool justCalcExtents)
{
	if (!justCalcExtents)
	{
		//
...
float FindStartingXPos( const wchar_t *text )
{
...
//
////////////////////////////////////////////////////////////////////////////////////
void	Render2DSentenceClass::Build_Sentence_Centered (const wchar_t *text, int *hkX, int *hkY)
{
	float char_height = Font->Get_Char_Height ();
	int		wordWidth = 0;
...
//
////////////////////////////////////////////////////////////////////////////////////
Vector2	Render2DSentenceClass::Build_Sentence_Not_Centered (const wchar_t *text, int *hkX, int *hkY, bool justCalcExtents)
{
...
////////////////////////////////////////////////////////////////////////////////////
void
Render2DSentenceClass::Build_Sentence (const wchar_t *text, int *hkX, int *hkY)
{
	if (text == nullptr) {
		return ;
	}
...
////////////////////////////////////////////////////////////////////////////////////
const FontCharsClassCharDataStruct *
FontCharsClass::Store_Backend_Char (wchar_t ch)
{
	IRenderBackend *backend = WW3D::Get_Render_Backend();
	if (backend == nullptr || BackendFont == nullptr) {
		return nullptr;
	}

	RenderBackendFontGlyph glyph = {};
	if (!backend->Get_Font_Glyph(BackendFont, static_cast<unsigned int>(ch), glyph) ||
		glyph.pixels == nullptr || glyph.width == 0 || glyph.height == 0 || glyph.pitch == 0) {
		return nullptr;
	}
	...
	const int char_width = static_cast<int>(glyph.width) + PixelOverlap + xOrigin;
	const int char_height = std::min(CharHeight, static_cast<int>(glyph.height));
	...
	//
	//	Copy the glyph contents to the buffer
	//
	for (int row = 0; row < char_height; row ++) {
		...
		for (int col = 0; col < char_width; col ++) {
			...
			//
			uint8 pixel_value = 0;
			if (col >= xOrigin && col < xOrigin + static_cast<int>(glyph.width)) {
				pixel_value = glyph.pixels[row * glyph.pitch + (col - xOrigin)];
			}
#ifdef TEST_PLACEMENT
 			...
 			if (row==1&&col==1) {
	 			...
	 			if (row==CharHeight-1&&col==char_width-1-PixelOverlap) {
	 				pixel_value = 0xff;
	 			}
	 			if (row==CharHeight-2&&col==char_width-2-PixelOverlap) {
	 				pixel_value = 0xff;
	 			}
	 			if (row==0&&col==char_width-1-PixelOverlap) {
	 				pixel_value = 0xff;
	 			}
	 			if (row==1&&col==char_width-2-PixelOverlap) {
	 				pixel_value = 0xff;
 			}
 			if (pixel_value == 0x00) {
 				pixel_value = 0x40;
...
////////////////////////////////////////////////////////////////////////////////////
bool
FontCharsClass::Create_Backend_Font (const char *font_name)
{
	IRenderBackend *backend = WW3D::Get_Render_Backend();
	if (backend == nullptr || font_name == nullptr) {
		return false;
	}

	const char *font_to_use_for_generals = "Arial";
	bool doingGenerals = false;
	if (strcmp(font_name, "Generals")==0) {
		font_name = font_to_use_for_generals;
		doingGenerals = true;
	}
	...
	//
	const int dots_per_inch = 96; // always use 96.
	const int font_height = -(PointSize * dots_per_inch) / 72;

	int font_width = 0; // use font default.
	if (doingGenerals) {
		//font_width = -font_height*0.35f; //2 pixels tighter.
		font_width = static_cast<int>(-font_height*0.40f); // one pixel tighter
	}
	...
	BackendFont = backend->Create_Font(-font_height, font_name, IsBold, font_width);
	if (BackendFont == nullptr) {
		return false;
	}

	RenderBackendFontMetrics metrics = {};
	if (!backend->Get_Font_Metrics(BackendFont, metrics)) {
		Free_Backend_Font();
		return false;
	}
	...
	CharOverhang = doingGenerals ? 0 : metrics.overhang;
	return CharHeight > 0;
}
...
////////////////////////////////////////////////////////////////////////////////////
void
FontCharsClass::Free_Backend_Font ()
{
	IRenderBackend *backend = WW3D::Get_Render_Backend();
	if (backend != nullptr && BackendFont != nullptr) {
		backend->Release_Font(BackendFont);
	}
	BackendFont = nullptr;
}
...
FontCharsClass::Initialize_Font (const char *font_name, int point_size, bool is_bold)
{
	...
	//
	return Create_Backend_Font (font_name);
}
...
FontCharsClass::Is_Font (const char *font_name, int point_size, bool is_bold)
{
	...
	//
	if (	(BackendFontName.Compare_No_Case (font_name) == 0) &&
