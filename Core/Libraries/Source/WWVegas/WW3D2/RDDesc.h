...
public:
	...
	// Used by the active render backend while it enumerates its devices.
	void Set_Device_Name(const char *name) { set_device_name(name); }
	void Set_Driver_Name(const char *name) { set_driver_name(name); }
	void Set_Driver_Version(const char *version) { set_driver_version(version); }
	void Reset_Resolution_List() { reset_resolution_list(); }
	void Add_Resolution(int width, int height, int bits) { add_resolution(width, height, bits); }
