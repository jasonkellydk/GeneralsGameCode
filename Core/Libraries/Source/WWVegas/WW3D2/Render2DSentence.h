...
public:
	...
	bool	Initialize_Font( const char *font_name, int point_size, bool is_bold );
...
private:
	...
	//	Private methods
	//
	bool							Create_Backend_Font( const char *font_name );
	void							Free_Backend_Font();
	const FontCharsClassCharDataStruct *	Store_Backend_Char( wchar_t ch );
	void							Update_Current_Buffer( int char_width );
	const FontCharsClassCharDataStruct	*	Get_Char_Data( wchar_t ch );
	...
	int									PixelOverlap;
	int									PointSize;
	StringClass							BackendFontName;
	RenderBackendFont *					BackendFont;
...
//	const Vector2 & Get_Cursor()						{ return Cursor; }
	...
	Vector2	Get_Formatted_Text_Extents( const wchar_t * text );
...
private:
	...
	void	Allocate_New_Surface (const wchar_t *text, bool justCalcExtents = false);
	void	Release_Pending_Surfaces ();
	void	Build_Sentence_Centered (const wchar_t *text, int *hkX, int *hkY);
	Vector2	Build_Sentence_Not_Centered (const wchar_t *text, int *hkX, int *hkY,bool justCalcExtents = false );
	//
	//	Private member data
	//
