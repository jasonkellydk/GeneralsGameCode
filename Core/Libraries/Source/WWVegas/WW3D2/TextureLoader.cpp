...
static RenderBackendTextureHandle Create_Texture(unsigned width, unsigned height, WW3DFormat format,
	MipCountType mip_levels, RenderBackendTexturePool pool = RenderBackendTexturePool::Managed,
	bool render_target = false)
{
	return WW3D::Get_Render_Backend()->Create_Texture_Handle_Pooled(
		width, height, format, mip_levels, pool, render_target);
}

static RenderBackendTextureHandle Create_Texture(const char *filename, MipCountType mip_levels)
{
	return WW3D::Get_Render_Backend()->Create_Texture_From_File_Handle(filename, mip_levels);
}

static RenderBackendSurface *Create_Surface(unsigned width, unsigned height, WW3DFormat format)
{
	return WW3D::Get_Render_Backend()->Create_System_Memory_Surface(width, height, format);
}
...
static RenderBackendTextureHandle Create_Volume_Texture(unsigned width, unsigned height, unsigned depth,
	WW3DFormat format, MipCountType mip_levels, RenderBackendTexturePool pool = RenderBackendTexturePool::Managed)
{
	return WW3D::Get_Render_Backend()->Create_Volume_Texture_Handle(
		width, height, depth, format, mip_levels, pool);
}
...
RenderBackendSurface *Load_Compressed_Surface(
	...
	WW3DFormat dest_format)
{
	...
	RenderBackendSurface *surface = Create_Surface(width, height, dest_format);
	if (surface == nullptr)
	{
		return nullptr;
	}

	RenderBackendSurfaceDescription description;
	RenderBackendLockedSurface locked_surface;
	if (!backend->Get_Surface_Description(surface, description) ||
		!backend->Lock_Surface(surface, locked_surface))
	{
		backend->Release_Surface(surface);
		return nullptr;
	}

	dds_file.Copy_Level_To_Surface(0, description.format, description.width,
		description.height, static_cast<unsigned char *>(locked_surface.bits),
		locked_surface.pitch);
	backend->Unlock_Surface(surface);
	return surface;
}
...
static bool Is_Format_Compressed(WW3DFormat texture_format,bool allow_compression)
{
	...
	if (texture_format!=WW3D_FORMAT_UNKNOWN) {
		if (!WW3D::Get_Render_Backend()->Supports_DXTC() || !allow_compression) {
	...
	compressed|=(
		...
		WW3D::Get_Render_Backend()->Supports_DXTC() &&
		allow_compression);
...
{
	return WW3D::Get_Render_Backend()->Is_Render_Thread();
}
...
{
	RenderBackendTextureLimits texture_limits;
	WW3D::Get_Render_Backend()->Get_Texture_Limits(texture_limits);
	...
	if (poweroftwowidth>texture_limits.max_width)
	{
		poweroftwowidth=texture_limits.max_width;
	}
	if (poweroftwoheight>texture_limits.max_height)
	{
		poweroftwoheight=texture_limits.max_height;
	}
	if (poweroftwodepth>texture_limits.max_volume_extent)
	{
		poweroftwodepth=texture_limits.max_volume_extent;
	}

	const unsigned maxTextureAspectRatio = texture_limits.max_aspect_ratio;
...
RenderBackendTextureHandle TextureLoader::Load_Thumbnail(const StringClass& filename, const Vector3& hsv_shift)//,WW3DFormat texture_format)
{
	...
	RenderBackendTextureHandle texture = Create_Texture(
		thumb->Get_Width(),
		thumb->Get_Height(),
		dest_format,
		MIP_LEVELS_ALL,
		RenderBackendTexturePool::Managed);
	IRenderBackend *backend = WW3D::Get_Render_Backend();
	if (texture == 0)
	{
		return MissingTexture::_Get_Missing_Texture();
	...
	unsigned level=0;
	RenderBackendTextureLock locked_rects[12] = {};
	const unsigned level_count = backend->Get_Texture_Level_Count(texture);
	WWASSERT(level_count<=12);

	// Lock all surfaces
	for (level=0;level<level_count;++level) {
		if (!backend->Lock_Texture(texture, level, locked_rects[level]))
		{
			for (unsigned unlock_level = 0; unlock_level < level; ++unlock_level)
			{
				backend->Unlock_Texture(texture, unlock_level);
			}
	...
	for (level=0;level+1<level_count;++level) {
		BitmapHandlerClass::Copy_Image_Generate_Mipmap(
			width,
			height,
			static_cast<unsigned char *>(locked_rects[level].bits),
			locked_rects[level].row_pitch,
			dest_format,
			src_surface,
			src_pitch,
			src_format,
			static_cast<unsigned char *>(locked_rects[level+1].bits),	// mipmap
			locked_rects[level+1].row_pitch,
			hsv);
		hsv=Vector3(0.0f,0.0f,0.0f);	// Only do the shift for the first level, as the mipmaps are based on it.

		src_format=dest_format;
		src_surface=static_cast<unsigned char *>(locked_rects[level].bits);
		src_pitch=locked_rects[level].row_pitch;
		width>>=1;
		height>>=1;
	}
...
//
// ----------------------------------------------------------------------------
RenderBackendSurface *TextureLoader::Load_Surface_Immediate(
	const StringClass& filename,
	...
	bool allow_compression)
{
	...
	if (compressed) {
		RenderBackendSurface *surface=Load_Compressed_Surface(filename,0,WW3D_FORMAT_UNKNOWN);
		if (surface) {
			return surface;
		}
	}
	...
	// The renderer uses image upside down compared to TGA
	...
	BitmapHandlerClass::Copy_Image(
		static_cast<unsigned char *>(locked_surface.bits),
		width,
		height,
		locked_surface.pitch,
		description.format,
...
void TextureLoader::Request_Thumbnail(TextureBaseClass *tc)
{
	...
	if (tc->Peek_Render_Backend_Texture()) {
		return;
	}
...
void TextureLoader::Flush_Pending_Load_Tasks()
{
	...
	// (Only the render thread can make the backend calls necessary
...
{
	// All backend resource operations must run from the render thread.
	WWASSERT(Is_Render_Thread());

	// load thumbnail texture
	RenderBackendTextureHandle texture = Load_Thumbnail(tc->Get_Full_Path(),tc->Get_HSV_Shift());

	// apply thumbnail to texture
	if (tc->Get_Asset_Type()==TextureBaseClass::TEX_REGULAR)
	{
		tc->Apply_New_Surface(texture, false);
	}
	else if (texture != 0)
	{
		WW3D::Get_Render_Backend()->Release_Texture_Handle(texture);
	}
}
...
bool TextureLoadTaskClass::Load()
{
	...
	WWASSERT(Peek_Render_Backend_Texture());
...
{
	WWASSERT(TextureLoader::Is_Render_Thread());
	WWASSERT(!BackendTexture);

	BackendTexture = MissingTexture::_Get_Missing_Texture();
	Apply(true);
}
...
void TextureLoadTaskClass::Apply(bool initialize)
{
	...
	Texture->Apply_New_Surface(BackendTexture, initialize);
...
{
	IRenderBackend *backend = WW3D::Get_Render_Backend();
	MipLevelCount = backend->Get_Texture_Level_Count(BackendTexture);
	if (MipLevelCount == 0 || MipLevelCount > MIP_LEVELS_MAX)
	{
		WWASSERT_PRINT(0, "Invalid texture mip level count");
		return false;
	}
	...
	for (unsigned int i = 0; i < MipLevelCount; ++i)
	{
		...
		const bool locked = backend->Lock_Texture(BackendTexture, i, locked_texture);
		if (locked && locked_texture.bits != nullptr)
		{
			LockedSurfacePtr[i] = static_cast<unsigned char *>(locked_texture.bits);
			LockedSurfacePitch[i] = locked_texture.row_pitch;
		}
		else
		{
			if (locked)
			{
				backend->Unlock_Texture(BackendTexture, i);
			}
			Unlock_Surfaces();
			return false;
		}
...
void TextureLoadTaskClass::Unlock_Surfaces()
{
	for (unsigned int i = 0; i < MipLevelCount; ++i)
	{
		...
		{
			WWASSERT(WW3D::Get_Render_Backend()->Is_Render_Thread());
			WW3D::Get_Render_Backend()->Unlock_Texture(BackendTexture, i);
		}
		LockedSurfacePtr[i] = nullptr;
	}
...
bool CubeTextureLoadTaskClass::Lock_Surfaces()
{
	...
	if (MipLevelCount == 0 || MipLevelCount > MIP_LEVELS_MAX)
	{
		WWASSERT_PRINT(0, "Invalid cube texture mip level count");
		return false;
	}
	...
	for (unsigned int f=0; f<6; f++)
	{
		...
		{
			RenderBackendTextureLock locked_texture;
			const bool locked = backend->Lock_Cube_Texture(BackendTexture,
				static_cast<RenderBackendCubeFace>(f), i, locked_texture);
			if (locked && locked_texture.bits != nullptr)
			{
				LockedCubeSurfacePtr[f][i] = static_cast<unsigned char *>(locked_texture.bits);
				LockedCubeSurfacePitch[f][i] = locked_texture.row_pitch;
			}
			else
			{
				if (locked)
				{
					backend->Unlock_Cube_Texture(BackendTexture,
						static_cast<RenderBackendCubeFace>(f), i);
				}
				Unlock_Surfaces();
				return false;
			}
...
void CubeTextureLoadTaskClass::Unlock_Surfaces()
{
	for (unsigned int f=0; f<6; f++)
	{
		for (unsigned int i = 0; i < MipLevelCount; ++i)
		{
			...
			{
				WWASSERT(WW3D::Get_Render_Backend()->Is_Render_Thread());
				WW3D::Get_Render_Backend()->Unlock_Cube_Texture(BackendTexture,
					static_cast<RenderBackendCubeFace>(f), i);
			}
			LockedCubeSurfacePtr[f][i] = nullptr;
		}
...
bool CubeTextureLoadTaskClass::Begin_Compressed_Load()
{
	...
	BackendTexture	= Create_Cube_Texture
	(
		Width,
		Height,
		Format,
...
bool VolumeTextureLoadTaskClass::Lock_Surfaces()
{
	IRenderBackend *backend = WW3D::Get_Render_Backend();
	if (MipLevelCount == 0 || MipLevelCount > MIP_LEVELS_MAX)
	{
		WWASSERT_PRINT(0, "Invalid volume texture mip level count");
		return false;
	}
	...
	{
		RenderBackendTextureLock locked_texture;
		const bool locked = backend->Lock_Volume_Texture(BackendTexture, i, locked_texture);
		if (locked && locked_texture.bits != nullptr)
		{
			LockedSurfacePtr[i] = static_cast<unsigned char *>(locked_texture.bits);
			LockedSurfacePitch[i] = locked_texture.row_pitch;
			LockedSurfaceSlicePitch[i] = locked_texture.slice_pitch;
		}
		else
		{
			if (locked)
			{
				backend->Unlock_Volume_Texture(BackendTexture, i);
			}
			Unlock_Surfaces();
			return false;
		}
...
void VolumeTextureLoadTaskClass::Unlock_Surfaces()
{
	for (unsigned int i = 0; i < MipLevelCount; ++i)
	{
		...
		{
			WWASSERT(WW3D::Get_Render_Backend()->Is_Render_Thread());
			WW3D::Get_Render_Backend()->Unlock_Volume_Texture(BackendTexture, i);
		}
		LockedSurfacePtr[i] = nullptr;
	}
...
bool VolumeTextureLoadTaskClass::Begin_Compressed_Load()
{
	...
	BackendTexture	= Create_Volume_Texture
	(
		Width,
		Height,
		Depth,
