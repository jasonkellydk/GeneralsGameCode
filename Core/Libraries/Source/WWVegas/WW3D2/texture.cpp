...
static unsigned Compute_Surface_Size_Bytes(const D3DSURFACE_DESC &desc)
{
	...
	const WW3DFormat ww3d_format = D3DFormat_To_WW3DFormat(desc.Format);
	return width * height * Get_Bytes_Per_Pixel(ww3d_format);
}
...
unsigned TextureClass::Get_Texture_Memory_Usage() const
{
	...
	for (unsigned i=0;i<Peek_D3D_Texture()->GetLevelCount();++i)
	{
		...
		size += Compute_Surface_Size_Bytes(desc);
	}
	return size;
}
