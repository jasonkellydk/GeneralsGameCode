...
static void Create_Hash_Name(StringClass& name, const StringClass& thumb_name)
{
	...
	WWASSERT(WW3DString::Compare_No_Case(&name[len-4],".tga") == 0 || WW3DString::Compare_No_Case(&name[len-4],".dds") == 0);
	name[len-4]='\0';
	WW3DString::To_Lower(name.Peek_Buffer());
}
...
void ThumbnailManagerClass::Add_Thumbnail_Manager(const char* thumbnail_filename)
{
	...
	WWASSERT(WW3DString::Compare_No_Case(thumbnail_filename,GLOBAL_THUMBNAIL_MANAGER_FILENAME));
